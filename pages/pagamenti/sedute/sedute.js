...
function btnFiltra() {
    $.post('post/sedute.php?pagination=0', parseFilter())
    .done(response => { loadView(response);});
}
function btnClean(){
    $.post('post/sedute.php?pagination=0', {} ).done(response => { loadView(response);});
}
function loadView(response){
    ...
    document.querySelectorAll('a.page-link').forEach(link => {
        link.addEventListener('click', event => {
            ...
            $.post('post/sedute.php?pagination=' + page, parseFilter()).done(response => loadView(response));
        });
    });
