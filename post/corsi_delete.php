<?php 
    ...
    $now=now('Y-m-d');
    if($now==$insert_at){
        Delete()->from('corsi')->where("id={$_REQUEST['id']}");
        Delete()->from('corsi_planning')->where("id_corso={$_REQUEST['id']}");
        Delete()->from('corsi_pagamenti')->where("id_corso={$_REQUEST['id']}");
    }
    else{
        Update('corsi')->set(['deleted'=>1])->where("id={$_REQUEST['id']}");
        Delete()->from('corsi_planning')->where("id_corso={$_REQUEST['id']} AND data >= '{$now}'");
        Delete()->from('corsi_pagamenti')->where("id_corso={$_REQUEST['id']} AND scadenza > '{$now}'");
    }