...
<div class="modal bg-dark bg-opacity-50 vh-100" id="<?php echo $_REQUEST['id_modal'];?>" data-bs-backdrop="static" style="display: none;" >
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header"><h4 class="modal-title">Sposta Colloquio</h4>
                <button type="button" class="btn-resize"  onclick="resize('#<?php echo $_REQUEST['id_modal'];?>')"></button>
                <button type="button" class="btn-close" onclick="closeModal(this);" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <?php 
                    $result=$id_seduta?Select('*')->from('colloquio_planning')->where("id={$id_seduta}")->first():[];
                    if(empty($result)){
                ...
                <div class="p-2">
                    <div class="mb-3">
                        ...
                        <select type="text"  name="id_terapista" <?php echo _attr_data($id_seduta);?> class="form-control text-center" value="<?php echo $_REQUEST['id_terapista']??'';?>" 
                            onchange="window.modalHandlers['sposta_colloquio'].change(this)">
                            <?php 
            ...
            <div class="modal-footer">
                <a href="#" class="btn btn-secondary" <?php echo _attr_data($id_seduta);?>onclick="window.modalHandlers['sposta_colloquio'].btnPresente(this,<?php echo $_REQUEST['id_terapista'].",'{$_REQUEST['data']}'"; ?>)">Presente</a>
                <a href="#" class="btn btn-dark" <?php echo _attr_data($id_seduta);?>onclick="window.modalHandlers['sposta_colloquio'].btnAssente(this,<?php echo $_REQUEST['id_terapista'].",'{$_REQUEST['data']}'"; ?>)">Assente</a>
                <a href="#" class="btn btn-primary" <?php echo _attr_data($id_seduta);?>onclick="window.modalHandlers['sposta_colloquio'].btnSalva(this)">Sposta Colloquio</a>
            </div>
        </div>
    </div>
</div>
<?php modal_script('sposta_colloquio'); ?>