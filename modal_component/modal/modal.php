    ...
    function _element($col,$select,$is_last){
        ...
        switch ($type) {
            case 'enum':{
                ...
                $ret.="<div class=\"m-1 ".($is_last?'flex-fill':'w-md-50')."\">";
            ...
            case 'varchar':{
                ...
                $value=$select[$col['COLUMN_NAME']]??'';
                $ret.="<div class=\"m-1 w-md-{$chars} ".($is_last?'flex-fill':'')."\">";
                $ret.="<label for=\"{$name}\" class=\"form-label \">{$name}</label>";
            ...
            case 'double':{
                $value=$select[$col['COLUMN_NAME']]??'';
                $ret.="<div class=\"m-1 ".($is_last?'flex-fill':'w-md-30')."\">";
                $ret.="<label for=\"{$name}\" class=\"form-label \">{$name}</label>";
            ...
            case 'timestamp':{
                ...
                $ret.="<div class=\"m-2 ".($is_last?'flex-fill':'w-md-20')."\">";
                $ret.="<label for=\"{$name}\" class=\"form-label \">{$name}</label>";
            ...
            case 'longtext':{
                $value=$select[$col['COLUMN_NAME']]??'';
                $ret.="<div class=\"m-2 ".($is_last?'flex-fill':'w-md-60')."\">";
                $ret.="<label for=\"{$name}\" class=\"form-label \">{$name}</label>";
                ...
                <div class="p-0"><?php 
                    ...
                    foreach($columns as $column){
                        if($column['COLUMN_NAME']!='id'){
                            if($col==$cols) echo "<div class='d-flex flex-column flex-md-row'>";
